...
class GameState:
    ...
    # returns our color
    def ourColor(self, agent):
        blue = self.getBlueTeamIndices()
        return 'blue' if agent.index in blue else 'red'

    # returns our team_mate state and a list of our opponents' state
    def teamMateAndOpponentState(self, agent, color):
        red = self.getRedTeamIndices()
        blue = self.getBlueTeamIndices()
        team_mate = []
        opponent = []
        for i in range(len(blue)):
            if color == 'blue':
                if blue[i] != agent.index:
                    team_mate.append(self.getAgentState(blue[i]))
                opponent.append(self.getAgentState(red[i]))
            elif color == 'red':
                if red[i] != agent.index:
                    team_mate.append(self.getAgentState(red[i]))
                opponent.append(self.getAgentState(blue[i]))
    ...
    ### function to preprocess the data for the network
    # return the matrices for the network's input
    def dataInput(self, agent):
        """
        ...
        for (i, j) in self.getBlueCapsules():
        ...
        self.reorderMatrixLikeDisplay(power_capsule)
        if color is 'red':
        ...
        if agent_state.scaredTimer == 0:
            pacman_friend[self.invert(self.getAgentPosition(agent.index))] = 1  # TODO: check superPacMan
        if team_mate_state.scaredTimer == 0:
            pacman_friend[self.invert(team_mate_state.getPosition())] = -1  # TODO
        ...
        if agent_state.scaredTimer > 0:
            scared_ghost_friend[self.invert(self.getAgentPosition(agent.index))] = 1
        if team_mate_state.scaredTimer > 0:
