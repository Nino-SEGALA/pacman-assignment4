...
class State(CaptureAgent):
    ...
    def bfsOnBoard(self, gameState, color, agent_pos, distance):
        ...
        gameState.reorderMatrixLikeDisplay(board)
        if color is 'red':
            gameState.invertMatrixForRed(board)
        ...
        while lookAt:
            ...
            if depth < distance:  # don't look further than distance
                neighbours = gameState.freeNeighbours(board, (u, v))
    ...
    def dataInput(self, gameState, agent):
        ...
        print()
        color = gameState.ourColor(self)
        print(color, self.index)
        agent_state = gameState.getAgentState(self.index)
        team_mate_state, opponent_state = gameState.teamMateAndOpponentState(self, color)
        width = gameState.getWalls().width  # width of the board (32)
        height = gameState.getWalls().height  # height of the board (16)
        ...
        walls = np.array([[int(gameState.getWalls()[i][j]) for i in range(width)]
                            for j in range(height)])
        gameState.reorderMatrixLikeDisplay(walls)
        if color is 'red':
            gameState.invertMatrixForRed(walls)
        print("walls")
        print(walls)
        ...
        gameState.reorderMatrixLikeDisplay(power_capsule)
        if color is 'red':
            gameState.invertMatrixForRed(food)
            gameState.invertMatrixForRed(power_capsule)
        # print("food")
        # print(food)
        # print("power_capsule")
        # print(power_capsule)
        ...
        if agent_state.scaredTimer == 0:
            pacman_friend[gameState.invert(gameState.getAgentPosition(self.index))] = 1  # TODO: check superPacMan
        if team_mate_state.scaredTimer == 0:
            pacman_friend[gameState.invert(team_mate_state.getPosition())] = -1  # TODO
        gameState.reorderMatrixLikeDisplay(pacman_friend)
        if color is 'red':
            gameState.invertMatrixForRed(pacman_friend)
        #print("pacman_friend")
        #print(pacman_friend)
        ...
        if agent_state.scaredTimer > 0:
            scared_ghost_friend[gameState.invert(gameState.getAgentPosition(self.index))] = 1
        if team_mate_state.scaredTimer > 0:
            scared_ghost_friend[gameState.invert(team_mate_state.getPosition())] = 1
        if color is 'red':
            gameState.invertMatrixForRed(scared_ghost_friend)
        gameState.reorderMatrixLikeDisplay(scared_ghost_friend)
        ...
        if opponent_state[0].scaredTimer == 0:
            ...
            if pos1:
                pacman_opponent[gameState.invert(pos1)] = 1  # TODO: check superPacMan
        if opponent_state[1].scaredTimer == 0:
            pos2 = opponent_state[1].getPosition()
            if pos2:
                pacman_opponent[gameState.invert(pos2)] = 1
        gameState.reorderMatrixLikeDisplay(pacman_opponent)
        if color is 'red':
            gameState.invertMatrixForRed(pacman_opponent)
        ...
        # print("common position = ", gameState.findCommonPosition(boxes1, boxes2))
        # print("case 2")
        boxes3 = gameState.bfsOnBoard(color, agent_pos, distToOp[1])
        boxes4 = gameState.bfsOnBoard(color, team_mate_pos, distToOpTeamMate[1])'''
        # print("common position = ", gameState.findCommonPosition(boxes3, boxes4))
        # SIMPLE TEST CASE
        ...
        gameState.reorderMatrixLikeDisplay(scared_ghost_opponent)
        if color is 'red':
            gameState.invertMatrixForRed(scared_ghost_opponent)
